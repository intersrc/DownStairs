...
var G = function() { };
G.__name__ = true;
G.stringTime = function(seconds) {
	var h = Math.floor(seconds / 3600);
	seconds -= h * 3600;
	var m = Math.floor(seconds / 60);
	seconds -= m * 60;
	var s = Math.floor(seconds);
	return G.addZero(h) + ":" + G.addZero(m) + ":" + G.addZero(s);
};
G.addZero = function(i) {
	if(i < 10) {
		return "0" + i;
	} else if(i == null) {
		return "null";
...
var Game = function() {
	...
	this.floorLayer = new createjs.Container();
	this.addChild(this.floorLayer);
	this.floors = [];
	var floor = new down_Floor(G.stageWidth / 2,G.stageHeight - 48,down_FloorType.Normal);
	this.floorLayer.addChild(floor.container);
	this.floors.push(floor);
	var arrow = new createjs.Bitmap("./assets/arrow_0.png");
	this.addChild(arrow);
	this.player = new down_Player(G.stageWidth / 2,G.stageHeight / 2 - 128);
	this.player.container.alpha = 0;
	createjs.Tween.get(this.player.container).to({ alpha : 1},500);
	this.addChild(this.player.container);
	this.layer = new createjs.Text("00:00:00","bold 24px Arial","#FFF");
	this.layer.x = 4;
	this.layer.y = G.stageHeight - 24 - 4;
	this.addChild(this.layer);
	G.totalTime = 0;
	this.exists = true;
	var _this = G.space.zpp_inner.wrap_listeners;
	...
	var obj1 = new nape_callbacks_InteractionListener(obj,zpp_$nape_util_ZPP_$Flags.InteractionType_COLLISION,down_Player.CB_TYPE,down_Floor.CB_TYPE_ELASTIC,function(cb) {
		createjs.Sound.play("elastic");
	});
...
Game.__name__ = true;
Game.__super__ = createjs.Container;
Game.prototype = $extend(createjs.Container.prototype,{
	destory: function() {
		var _g = 0;
		var _g1 = this.floors;
		while(_g < _g1.length) {
	...
	,update: function(time) {
		if(this.exists) {
			G.totalTime += time;
			this.layer.text = G.stringTime(G.totalTime);
			G.space.step(time);
			var _g1 = 0;
			var _g = this.floors.length;
			while(_g1 < _g) {
				var i = _g1++;
				var floor = this.floors[i];
				if(floor != null) {
					floor.update();
					var _this = floor.body;
					...
					var _this3 = floor.body.zpp_inner.aabb.wrapper();
					var _this4 = _this3.zpp_inner;
					if(_this4._validate != null) {
						_this4._validate();
					}
					var _this5 = _this3.zpp_inner;
					if(tmp < -(_this5.maxy - _this5.miny)) {
						floor.destroy();
						this.floors[i] = null;
					}
				}
			...
			if(this.player.exists) {
				...
				if(!(_this7.zpp_inner.y >= G.stageHeight)) {
					var _this9 = this.player.body;
			...
			this.createCd += time;
			if(this.createCd >= this.createCdMax) {
				this.createCd -= this.createCdMax;
				var floor1 = new down_Floor(Math.random() * G.stageWidth,G.stageHeight + 48);
				this.floorLayer.addChild(floor1.container);
				var _g11 = 0;
				var _g2 = this.floors.length + 1;
				while(_g11 < _g2) {
					var i1 = _g11++;
					if(this.floors[i1] == null) {
						this.floors[i1] = floor1;
						break;
					}
				}
			}
...
Main.main = function() {
	...
	G.stageWidth = 320;
	G.stageHeight = 568;
	var stage = new createjs.Stage(window.document.getElementById("mainCanvas"));
	G.stage = stage;
	var ret;
	...
	var gravity = ret;
	G.space = new nape_space_Space(gravity);
	var mute = new Mute();
	G.reset = function() {
		if(G.game != null) {
			G.game.destory();
		...
		G.space.zpp_inner.wrap_listeners.clear();
...
var Menu = function(endCallback) {
	...
	if(Menu.first) {
		Menu.first = false;
		var text = new createjs.Text("点击开始","48px 黑体","#FFFFFF");
		text.textAlign = "center";
		text.lineHeight = 64;
		text.x = G.stageWidth / 2;
		text.y = (G.stageHeight - text.getBounds().height) / 2;
		this.addChild(text);
		var gap_big = (G.stageWidth / 2 - 96) / 2 | 0;
		var gap_mid = gap_big / 2 | 0;
		...
		this.addChild(shape);
		var left = new createjs.Bitmap("./assets/arrow_left_1.png");
		left.x = gap_big;
		left.y = shape.y + gap_big;
		left.alpha = 0.5;
		this.addChild(left);
		var right = new createjs.Bitmap("./assets/arrow_right_1.png");
		...
		var text1 = new createjs.Text("你坚持了\n" + G.stringTime(G.totalTime),"48px 黑体","#FFFFFF");
		text1.textAlign = "center";
		text1.lineHeight = 64;
		text1.x = G.stageWidth / 2;
		text1.y = (G.stageHeight - text1.getBounds().height) / 2;
		text1.addEventListener("click",function(event1) {
			endCallback();
		});
		this.addChild(text1);
		background.addEventListener("click",function(event2) {
			endCallback();
		});
		this.y = -G.stageHeight;
		createjs.Tween.get(this).to({ y : 0},1000,createjs.Ease.bounceOut);
	}
};
Menu.__name__ = true;
Menu.__super__ = createjs.Container;
Menu.prototype = $extend(createjs.Container.prototype,{
	destroy: function() {
	}
	,__class__: Menu
});
var Mute = function() {
	...
	if(createjs.Sound.initializeDefaultPlugins()) {
		var normal = [0,1,"normal"];
		var data = { images : ["./assets/mute_0.png"], frames : { width : 32, height : 32, count : 2}, animations : { normal : normal}};
		var spriteSheet = new createjs.SpriteSheet(data);
		var sprite = new createjs.Sprite(spriteSheet,"normal");
		sprite.alpha = 0.49;
		sprite.x = G.stageWidth - 32 - 8;
		sprite.y = 8;
		var tmp = createjs.Sound.getMute() ? 1 : 0;
		sprite.gotoAndStop(tmp);
		this.addChild(sprite);
		var bg = new createjs.Bitmap("./assets/mute_bg_0.png");
		bg.x = sprite.x;
		bg.y = sprite.y;
		bg.alpha = 0.01;
		this.addChild(bg);
		bg.addEventListener("click",function() {
			...
			var tmp1 = createjs.Sound.getMute() ? 1 : 0;
			sprite.gotoAndStop(tmp1);
		});
	}
};
Mute.__name__ = true;
Mute.__super__ = createjs.Container;
Mute.prototype = $extend(createjs.Container.prototype,{
	__class__: Mute
});
var Std = function() { };
Std.__name__ = true;
...
var down_FloorType = { __ename__ : true, __constructs__ : ["Random","Normal","Elastic","Conveyer"] };
down_FloorType.Random = ["Random",0];
down_FloorType.Random.toString = $estr;
down_FloorType.Random.__enum__ = down_FloorType;
down_FloorType.Normal = ["Normal",1];
down_FloorType.Normal.toString = $estr;
down_FloorType.Normal.__enum__ = down_FloorType;
down_FloorType.Elastic = ["Elastic",2];
down_FloorType.Elastic.toString = $estr;
down_FloorType.Elastic.__enum__ = down_FloorType;
down_FloorType.Conveyer = ["Conveyer",3];
down_FloorType.Conveyer.toString = $estr;
down_FloorType.Conveyer.__enum__ = down_FloorType;
var game_Actor = function(body,display) {
	this.linearDragY = 1;
	this.linearDragX = 1;
	this.body = body;
	...
	var tempX = _this1.zpp_inner.x;
	var _this3 = this.body;
	...
	var tempY = _this4.zpp_inner.y;
	var tempRotation = this.body.zpp_inner.rot;
	var _this6 = this.body;
	if(_this6.zpp_inner.wrap_pos == null) {
		_this6.zpp_inner.setupPosition();
	}
	var _this7 = _this6.zpp_inner.wrap_pos;
	var _this8 = this.body;
	if(_this8.zpp_inner.wrap_pos == null) {
		_this8.zpp_inner.setupPosition();
	}
	var _this9 = _this8.zpp_inner.wrap_pos;
	var _this10 = _this9.zpp_inner;
	var _this11 = _this9.zpp_inner;
	if(_this11._validate != null) {
		_this11._validate();
	}
	if(_this9.zpp_inner.y != 0) {
		_this9.zpp_inner.y = 0;
	...
	var x = _this9.zpp_inner.y;
	var _this14 = _this7.zpp_inner;
	var _this15 = _this7.zpp_inner;
	if(_this15._validate != null) {
		_this15._validate();
	}
	if(_this7.zpp_inner.x != x) {
		_this7.zpp_inner.x = x;
		var _this16 = _this7.zpp_inner;
		if(_this16._invalidate != null) {
			_this16._invalidate(_this16);
		}
	}
	var _this17 = _this7.zpp_inner;
	if(_this17._validate != null) {
		_this17._validate();
	}
	var _this18 = this.body;
	_this18.zpp_inner.immutable_midstep("Body::rotation");
	if(_this18.zpp_inner.rot != 0) {
		_this18.zpp_inner.rot = 0;
		var _this19 = _this18.zpp_inner;
		_this19.zip_axis = true;
		var cx_ite = _this19.shapes.head;
		while(cx_ite != null) {
			var s = cx_ite.elt;
			if(s.type == 1) {
		...
		_this19.zip_worldCOM = true;
		_this18.zpp_inner.wake();
	}
	var _this20 = this.body.zpp_inner.aabb.wrapper();
	var _this21 = _this20.zpp_inner;
	if(_this21._validate != null) {
		_this21._validate();
	}
	var centerX = _this20.zpp_inner.minx;
	var _this22 = this.body.zpp_inner.aabb.wrapper();
	var _this23 = _this22.zpp_inner;
	if(_this23._validate != null) {
		_this23._validate();
	}
	var _this24 = _this22.zpp_inner;
	var centerX1 = centerX + (_this24.maxx - _this24.minx) / 2;
	var _this25 = this.body.zpp_inner.aabb.wrapper();
	var _this26 = _this25.zpp_inner;
	if(_this26._validate != null) {
		_this26._validate();
	}
	var centerY = _this25.zpp_inner.miny;
	var _this27 = this.body.zpp_inner.aabb.wrapper();
	var _this28 = _this27.zpp_inner;
	if(_this28._validate != null) {
		_this28._validate();
	}
	var _this29 = _this27.zpp_inner;
	var centerY1 = centerY + (_this29.maxy - _this29.miny) / 2;
	display.x = centerX1 - display.getBounds().width / 2;
	display.y = centerY1 - display.getBounds().height / 2;
	this.container.addChild(display);
	var _this30 = this.body;
	...
	var _this33 = _this31.zpp_inner;
	if(_this33._validate != null) {
		_this33._validate();
	}
	if(_this31.zpp_inner.x != tempX) {
		_this31.zpp_inner.x = tempX;
		var _this34 = _this31.zpp_inner;
		if(_this34._invalidate != null) {
			_this34._invalidate(_this34);
		}
	}
	var _this35 = _this31.zpp_inner;
	if(_this35._validate != null) {
		_this35._validate();
	}
	var _this36 = this.body;
	if(_this36.zpp_inner.wrap_pos == null) {
		_this36.zpp_inner.setupPosition();
	}
	var _this37 = _this36.zpp_inner.wrap_pos;
	var _this38 = _this37.zpp_inner;
	var _this39 = _this37.zpp_inner;
	if(_this39._validate != null) {
		_this39._validate();
	}
	if(_this37.zpp_inner.y != tempY) {
		_this37.zpp_inner.y = tempY;
		var _this40 = _this37.zpp_inner;
		if(_this40._invalidate != null) {
			_this40._invalidate(_this40);
		}
	}
	var _this41 = _this37.zpp_inner;
	if(_this41._validate != null) {
		_this41._validate();
	}
	var _this42 = this.body;
	_this42.zpp_inner.immutable_midstep("Body::rotation");
	if(_this42.zpp_inner.rot != tempRotation) {
		_this42.zpp_inner.rot = tempRotation;
		var _this43 = _this42.zpp_inner;
		_this43.zip_axis = true;
		var cx_ite1 = _this43.shapes.head;
		while(cx_ite1 != null) {
			var s1 = cx_ite1.elt;
			if(s1.type == 1) {
		...
		_this43.zip_worldCOM = true;
		_this42.zpp_inner.wake();
	}
	this.exists = true;
	this.update();
};
game_Actor.__name__ = true;
game_Actor.prototype = {
	update: function() {
		if(this.exists) {
			...
			this.container.x = _this1.zpp_inner.x;
			var _this3 = this.body;
			if(_this3.zpp_inner.wrap_pos == null) {
				_this3.zpp_inner.setupPosition();
			}
			...
			this.container.y = _this4.zpp_inner.y;
			if(!this.body.zpp_inner.norotate) {
				this.container.rotation = this.body.zpp_inner.rot * 180 / Math.PI;
			}
			if(this.linearDragX < 1) {
				var _this6 = this.body;
				if(_this6.zpp_inner.wrap_vel == null) {
					_this6.zpp_inner.setupVelocity();
				}
				var _g = _this6.zpp_inner.wrap_vel;
				var _this7 = _g.zpp_inner;
				if(_this7._validate != null) {
					_this7._validate();
				}
				var x = _g.zpp_inner.x * this.linearDragX;
				var _this8 = _g.zpp_inner;
			...
			if(this.linearDragY < 1) {
				var _this12 = this.body;
				if(_this12.zpp_inner.wrap_vel == null) {
					_this12.zpp_inner.setupVelocity();
				}
				var _g1 = _this12.zpp_inner.wrap_vel;
				var _this13 = _g1.zpp_inner;
				if(_this13._validate != null) {
					_this13._validate();
				}
				var y = _g1.zpp_inner.y * this.linearDragY;
	...
	,destroy: function() {
		this.exists = false;
		if(this.body != null) {
			G.space.zpp_inner.wrap_bodies.remove(this.body);
			this.body = null;
		}
...
var down_Floor = function(x,y,type) {
	if(zpp_$nape_util_ZPP_$Flags.BodyType_KINEMATIC == null) {
		zpp_$nape_util_ZPP_$Flags.internal = true;
		zpp_$nape_util_ZPP_$Flags.BodyType_KINEMATIC = new nape_phys_BodyType();
		zpp_$nape_util_ZPP_$Flags.internal = false;
	}
	var tBody = new nape_phys_Body(zpp_$nape_util_ZPP_$Flags.BodyType_KINEMATIC);
	var tDO = null;
	var w = 128;
	var h = 16;
	var r = 32;
	if(type == null || Type.enumEq(type,down_FloorType.Random)) {
		var rand = Math.random();
		if(rand < 0.2) {
			type = down_FloorType.Elastic;
		} else if(rand < 0.35) {
			type = down_FloorType.Conveyer;
		} else {
			type = down_FloorType.Normal;
		}
	}
	switch(type[1]) {
	case 0:
		break;
	case 1:
		var _this = tBody.zpp_inner.wrap_shapes;
		var obj = new nape_shape_Polygon(nape_shape_Polygon.box(w,h));
		if(_this.zpp_inner.reverse_flag) {
			_this.push(obj);
		} else {
			_this.unshift(obj);
		}
		tBody.setShapeMaterials(nape_phys_Material.steel());
		if(tBody.zpp_inner_i.wrap_cbTypes == null) {
			tBody.zpp_inner_i.setupcbTypes();
		}
		var _this1 = tBody.zpp_inner_i.wrap_cbTypes;
		var obj1 = down_Floor.CB_TYPE;
		if(_this1.zpp_inner.reverse_flag) {
			_this1.push(obj1);
		} else {
			_this1.unshift(obj1);
		}
		tDO = new createjs.Bitmap("./assets/floor_0.png");
		...
		rand1 /= 5;
		var angularVel = Math.PI * 0.25 * (rand1 - 0.5);
		if(tBody.zpp_inner.angvel != angularVel) {
			tBody.zpp_inner.angvel = angularVel;
			tBody.zpp_inner.wake();
		}
		break;
	case 2:
		var _this2 = tBody.zpp_inner.wrap_shapes;
		var obj2 = new nape_shape_Polygon(nape_shape_Polygon.box(w,h));
		if(_this2.zpp_inner.reverse_flag) {
			_this2.push(obj2);
		} else {
			_this2.unshift(obj2);
		}
		var material = nape_phys_Material.rubber();
		if(2 != material.zpp_inner.elasticity) {
			material.zpp_inner.elasticity = 2.;
			material.zpp_inner.invalidate(zpp_$nape_phys_ZPP_$Material.WAKE | zpp_$nape_phys_ZPP_$Material.ARBITERS);
		}
		tBody.setShapeMaterials(material);
		if(tBody.zpp_inner_i.wrap_cbTypes == null) {
			tBody.zpp_inner_i.setupcbTypes();
		}
		var _this3 = tBody.zpp_inner_i.wrap_cbTypes;
		var obj3 = down_Floor.CB_TYPE_ELASTIC;
		if(_this3.zpp_inner.reverse_flag) {
			_this3.push(obj3);
		} else {
			_this3.unshift(obj3);
		}
		tDO = new createjs.Bitmap("./assets/floor_1.png");
		tDO.setBounds(0,0,w,h);
		break;
	case 3:
		var _this4 = tBody.zpp_inner.wrap_shapes;
		var obj4 = new nape_shape_Polygon(nape_shape_Polygon.box(w,h));
		if(_this4.zpp_inner.reverse_flag) {
			_this4.push(obj4);
		} else {
			_this4.unshift(obj4);
		}
		var material1 = nape_phys_Material.steel();
		if(1.4 != material1.zpp_inner.dynamicFriction) {
			material1.zpp_inner.dynamicFriction = 1.4;
			material1.zpp_inner.invalidate(zpp_$nape_phys_ZPP_$Material.WAKE | zpp_$nape_phys_ZPP_$Material.ANGDRAG | zpp_$nape_phys_ZPP_$Material.ARBITERS);
		}
		if(1.4 != material1.zpp_inner.staticFriction) {
			material1.zpp_inner.staticFriction = 1.4;
			material1.zpp_inner.invalidate(zpp_$nape_phys_ZPP_$Material.WAKE | zpp_$nape_phys_ZPP_$Material.ARBITERS);
		}
		tBody.setShapeMaterials(material1);
		if(tBody.zpp_inner.wrap_svel == null) {
			tBody.zpp_inner.setupsvel();
		}
		var _this5 = tBody.zpp_inner.wrap_svel;
		var x1 = Std.random(2) == 0 ? -300 : 300;
		var _this6 = _this5.zpp_inner;
		var _this7 = _this5.zpp_inner;
		if(_this7._validate != null) {
			_this7._validate();
		}
		...
		var _this10 = tBody.zpp_inner_i.wrap_cbTypes;
		var obj5 = down_Floor.CB_TYPE;
		if(_this10.zpp_inner.reverse_flag) {
			_this10.push(obj5);
		} else {
			_this10.unshift(obj5);
		}
		var normal = [0,1,"normal"];
		var data = { images : ["./assets/floor_3.png"], frames : { width : w, height : h, count : 2}, animations : { normal : normal}};
		var spriteSheet = new createjs.SpriteSheet(data);
		tDO = new createjs.Sprite(spriteSheet,"normal");
		var sprite = tDO;
		sprite.framerate = 10.;
		tDO.setBounds(0,0,w,h);
		break;
	}
	if(tBody.zpp_inner.wrap_pos == null) {
		tBody.zpp_inner.setupPosition();
	}
	var _this11 = tBody.zpp_inner.wrap_pos;
	var _this12 = _this11.zpp_inner;
	var tmp;
	var _this13 = _this11.zpp_inner;
	if(_this13._validate != null) {
		_this13._validate();
	}
	if(_this11.zpp_inner.x == x) {
		var _this14 = _this11.zpp_inner;
		if(_this14._validate != null) {
			_this14._validate();
		}
		tmp = _this11.zpp_inner.y == y;
	} else {
		tmp = false;
	}
	if(!tmp) {
		_this11.zpp_inner.x = x;
		_this11.zpp_inner.y = y;
		var _this15 = _this11.zpp_inner;
		if(_this15._invalidate != null) {
			_this15._invalidate(_this15);
		}
	}
	var space = G.space;
	tBody.zpp_inner.immutable_midstep("Body::space");
	if((tBody.zpp_inner.space == null ? null : tBody.zpp_inner.space.outer) != space) {
		if((tBody.zpp_inner.space == null ? null : tBody.zpp_inner.space.outer) != null) {
			tBody.zpp_inner.component.woken = false;
		}
		if((tBody.zpp_inner.space == null ? null : tBody.zpp_inner.space.outer) != null) {
			(tBody.zpp_inner.space == null ? null : tBody.zpp_inner.space.outer).zpp_inner.wrap_bodies.remove(tBody);
		}
		if(space != null) {
			var _this16 = space.zpp_inner.wrap_bodies;
			if(_this16.zpp_inner.reverse_flag) {
				_this16.push(tBody);
			} else {
				_this16.unshift(tBody);
			}
		}
	}
	var tmp1 = tBody.zpp_inner.space == null;
	game_Actor.call(this,tBody,tDO);
	this.body.setShapeFilters(new nape_dynamics_InteractionFilter(2,4));
	var _this17 = this.body;
	if(_this17.zpp_inner.wrap_vel == null) {
		_this17.zpp_inner.setupVelocity();
	}
	var _this18 = _this17.zpp_inner.wrap_vel;
	var _this19 = _this18.zpp_inner;
	var _this20 = _this18.zpp_inner;
	if(_this20._validate != null) {
		_this20._validate();
	}
	if(_this18.zpp_inner.y != -60) {
		_this18.zpp_inner.y = -60;
		var _this21 = _this18.zpp_inner;
...
down_Floor.__name__ = true;
down_Floor.__super__ = game_Actor;
down_Floor.prototype = $extend(game_Actor.prototype,{
...
var down_Player = function(x,y) {
	...
	var rect = new nape_phys_Body(zpp_$nape_util_ZPP_$Flags.BodyType_DYNAMIC);
	var _this = rect.zpp_inner.wrap_shapes;
	var obj = new nape_shape_Polygon(nape_shape_Polygon.box(w,h));
	if(_this.zpp_inner.reverse_flag) {
		_this.push(obj);
	} else {
	...
	if(rect.zpp_inner.wrap_pos == null) {
		rect.zpp_inner.setupPosition();
	}
	var _this1 = rect.zpp_inner.wrap_pos;
	var _this2 = _this1.zpp_inner;
	var tmp;
	...
	rect.zpp_inner.immutable_midstep("Body::" + "false");
	if(!rect.zpp_inner.norotate != false) {
		rect.zpp_inner.norotate = true;
		rect.zpp_inner.invalidate_inertia();
	}
	rect.setShapeMaterials(nape_phys_Material.wood());
	var space = G.space;
	rect.zpp_inner.immutable_midstep("Body::space");
	if((rect.zpp_inner.space == null ? null : rect.zpp_inner.space.outer) != space) {
		if((rect.zpp_inner.space == null ? null : rect.zpp_inner.space.outer) != null) {
			rect.zpp_inner.component.woken = false;
		}
		if((rect.zpp_inner.space == null ? null : rect.zpp_inner.space.outer) != null) {
			(rect.zpp_inner.space == null ? null : rect.zpp_inner.space.outer).zpp_inner.wrap_bodies.remove(rect);
		}
		if(space != null) {
	...
	var tmp1 = rect.zpp_inner.space == null;
	var right_run = [0,3,"right_run"];
	var right_stand = [0,0,"right_stand"];
	var right_jump = [3,3,"right_jump"];
	var left_run = [4,7,"left_run"];
	var left_stand = [4,4,"left_stand"];
	var left_jump = [7,7,"left_jump"];
	var data = { images : ["./assets/player_3.png"], frames : { width : frameSize, height : frameSize, count : 8}, animations : { right_run : right_run, right_stand : right_stand, right_jump : right_jump, left_run : left_run, left_stand : left_stand, left_jump : left_jump}};
	var spriteSheet = new createjs.SpriteSheet(data);
	this.sprite = new createjs.Sprite(spriteSheet,"right_jump");
	this.sprite.framerate = 10.;
	this.sprite.setBounds(0,0,frameSize,frameSize + (frameSize - h));
	game_Actor.call(this,rect,this.sprite);
	this.linearDragX = 0.96;
	this.body.setShapeFilters(new nape_dynamics_InteractionFilter(4,2));
	var _this7 = this.body;
	if(_this7.zpp_inner_i.wrap_cbTypes == null) {
		_this7.zpp_inner_i.setupcbTypes();
	}
	var _this8 = _this7.zpp_inner_i.wrap_cbTypes;
	var obj1 = down_Player.CB_TYPE;
...
down_Player.prototype = $extend(game_Actor.prototype,{
	...
	,die: function() {
		if(this.exists) {
			this.exists = false;
			var tmp = this.container.rotation + 720;
			createjs.Tween.get(this.container).to({ alpha : 0, visible : false, rotation : tmp, scaleX : 2, scaleY : 2},500).call(G.reset);
	...
	,update: function() {
		...
		if(this.exists) {
			if(this.mousePressed && this.stageMouseY > 48) {
				...
				ret.zpp_inner.weak = true;
				tmp.applyImpulse(ret);
			}
			var _this4 = this.body;
			if(_this4.zpp_inner.wrap_vel == null) {
				_this4.zpp_inner.setupVelocity();
			}
			var _this5 = _this4.zpp_inner.wrap_vel;
			var _this6 = _this5.zpp_inner;
			if(_this6._validate != null) {
				_this6._validate();
			}
			if(Math.abs(_this5.zpp_inner.x) > 2) {
				if(this.facingLeft) {
					if(this.sprite.currentAnimation != "left_run") {
						this.sprite.gotoAndPlay("left_run");
					}
				} else if(this.sprite.currentAnimation != "right_run") {
					this.sprite.gotoAndPlay("right_run");
				}
			} else if(this.facingLeft) {
				if(this.sprite.currentAnimation != "left_stand") {
					this.sprite.gotoAndPlay("left_stand");
				}
			} else if(this.sprite.currentAnimation != "right_stand") {
				this.sprite.gotoAndPlay("right_stand");
			}
			var _this7 = this.body;
			...
			if(_this8.zpp_inner.x < 16) {
				var _this10 = this.body;
				if(_this10.zpp_inner.wrap_pos == null) {
					_this10.zpp_inner.setupPosition();
				}
				var _this11 = _this10.zpp_inner.wrap_pos;
				var _this12 = _this11.zpp_inner;
				var _this13 = _this11.zpp_inner;
				if(_this13._validate != null) {
					_this13._validate();
				}
				if(_this11.zpp_inner.x != 16) {
					_this11.zpp_inner.x = 16;
					var _this14 = _this11.zpp_inner;
				...
				if(_this17.zpp_inner.x > G.stageWidth - 16) {
					var _this19 = this.body;
					if(_this19.zpp_inner.wrap_pos == null) {
						_this19.zpp_inner.setupPosition();
					}
					var _this20 = _this19.zpp_inner.wrap_pos;
					var x1 = G.stageWidth - 16;
					var _this21 = _this20.zpp_inner;
					var _this22 = _this20.zpp_inner;
					if(_this22._validate != null) {
						_this22._validate();
					}
					if(_this20.zpp_inner.x != x1) {
						_this20.zpp_inner.x = x1;
						var _this23 = _this20.zpp_inner;
						if(_this23._invalidate != null) {
							_this23._invalidate(_this23);
						}
					}
					var _this24 = _this20.zpp_inner;
...
nape_dynamics_Arbiter.prototype = {
	toString: function() {
		...
		if(this.zpp_inner.cleared) {
			...
			var tmp = ret + "(" + (this.zpp_inner.ws1.id > this.zpp_inner.ws2.id ? this.zpp_inner.ws2.outer : this.zpp_inner.ws1.outer).toString() + "|" + (this.zpp_inner.ws1.id > this.zpp_inner.ws2.id ? this.zpp_inner.ws1.outer : this.zpp_inner.ws2.outer).toString() + ")";
			var tmp1 = this.zpp_inner.type == zpp_$nape_dynamics_ZPP_$Arbiter.COL ? "[" + ["SD","DD"][this.zpp_inner.colarb.stat ? 0 : 1] + "]" : "";
...
nape_phys_Body.prototype = $extend(nape_phys_Interactor.prototype,{
	applyImpulse: function(impulse,pos,sleepable) {
		...
		var tmp = this.zpp_inner;
		var tmp1 = tmp.velx;
		var _this2 = impulse.zpp_inner;
		if(_this2._validate != null) {
			_this2._validate();
		}
		tmp.velx = tmp1 + impulse.zpp_inner.x * t;
		var tmp2 = this.zpp_inner;
		var tmp3 = tmp2.vely;
		var _this3 = impulse.zpp_inner;
		if(_this3._validate != null) {
			_this3._validate();
		}
		tmp2.vely = tmp3 + impulse.zpp_inner.y * t;
		if(pos != null) {
			var rx = 0.0;
			var ry = 0.0;
			...
			var tmp4 = this.zpp_inner;
			var tmp5 = tmp4.angvel;
			var _this6 = impulse.zpp_inner;
			if(_this6._validate != null) {
				_this6._validate();
			}
			var tmp6 = impulse.zpp_inner.y * rx;
			var _this7 = impulse.zpp_inner;
			if(_this7._validate != null) {
				_this7._validate();
			}
			tmp4.angvel = tmp5 + (tmp6 - impulse.zpp_inner.x * ry) * this.zpp_inner.iinertia;
			if(pos.zpp_inner.weak) {
		...
		if(impulse.zpp_inner.weak) {
			...
			impulse.zpp_inner = null;
			var o6 = impulse;
			o6.zpp_pool = zpp_$nape_util_ZPP_$PubPool.poolVec2;
...
down_Floor.CB_TYPE = new nape_callbacks_CbType();
down_Floor.CB_TYPE_ELASTIC = new nape_callbacks_CbType();
down_Player.CB_TYPE = new nape_callbacks_CbType();
